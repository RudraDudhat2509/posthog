...
export const runningTimeLogic = kea<runningTimeLogicType>([
    ...
    loaders(({ values }) => ({
        ...
        manualPreview: [
            ...
            {
                loadManualPreview: async (input: RunningTimeCalculationInputApi) => {
    ...
    selectors({
        ...
        automaticCalculationInput: [
            (s) => [s.isManualMode, s.orderedPrimaryMetricsWithResults, s.numberOfVariants, s.mde],
        ...
        targetSampleSize: [
            ...
            (isManualMode, experiment, automaticInput, automaticCalculation): number | null => {
                if (isManualMode) {
                    ...
                    return experiment?.parameters?.recommended_sample_size ?? null
                }
                if (!automaticInput) {
                    return null
                }
