...
class TestBackfill(BaseTest):
    def test_seeds_stages_without_celebrations_and_skips_streak(self) -> None:
        evaluators = _evaluators(loyal_days=lambda ctx: 30, cumulative_pageviews=lambda ctx: 1_000_000)
    ...
    def test_backfill_leaves_last_computed_at_unset(self) -> None:
        ...
        with patch.object(backfill, "EVALUATORS", _evaluators(loyal_days=lambda ctx: 5)):
            backfill.backfill_team(self.team.id)
        loyal = WebAnalyticsAchievementProgress.objects.for_team(self.team.id).get(
            user=self.user, track_key="loyal_hog"
        )
