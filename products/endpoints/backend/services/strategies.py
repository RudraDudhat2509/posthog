...
class InsightEndpointStrategy(EndpointQueryStrategy):
    ...
    def materialized_filters_override_satisfies_required(self, data: EndpointRunRequest) -> bool:
        # Mirror apply_materialized_filters: only the first property carrying a non-null value is
        # applied, and only if it yields a breakdown condition. An override that adds no WHERE
        # clause must not be treated as satisfying the required breakdown.
