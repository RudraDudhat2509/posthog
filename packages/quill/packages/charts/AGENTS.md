...
- `MetricCard` is a left-aligned tile by default; `align="center"` centers title/headline/subtitle (use it for a bare headline with no title, pill, or sparkline — the trends "Number" insight centers the value until a toggle adds chrome, then switches to `left`). Pass `title={null}` to drop the title row — the header band collapses when there is no title and no change pill, and the subtitle row is omitted when there is no subtitle (and no `labels`), so a value-only card renders just the number. `headline={(formattedValue) => node}` replaces the default `text-4xl` headline — use it to attach click handlers or custom typography (the trends "Number" insight wires the persons-modal click onto it). `headline` receives the hover-animated value when a sparkline is present, so it composes with `data`. `changeSize="md"` renders a larger change pill (default `sm`); `changeInline` puts the pill beside the headline instead of in the header; `sparklineFill` makes the sparkline fill the card's remaining height instead of a fixed `sparklineHeight`.
